...
from transformers.models.bert.modeling_bert import load_tf_weights_in_bert, BertPooler, BertOnlyMLMHead, \
    BertEmbeddings, \
    BertOnlyNSPHead, BertEncoder, BertForMaskedLM, BertPreTrainedModel, BertModel
...
class UniLmForGeneration(BertPreTrainedModel):
    ...
    @staticmethod
    def create_mask(attention_mask, token_type_ids):
        assert attention_mask.dim() == 2, "attention_mask dim must be 2 like [batch_size,seq_len]"
        raw_attention_mask = attention_mask.unsqueeze(-1) * attention_mask.unsqueeze(1)
        # TODO 确定 token_type_ids 的具体id是什么
        src_attention_mask = attention_mask & (token_type_ids == 0)
        target_attention_mask = attention_mask & (token_type_ids == 1)
        src_attention = src_attention_mask.unsqueeze(-1) * src_attention_mask.unsqueeze(1)
        target_attention = target_attention_mask.unsqueeze(-1) * target_attention_mask.unsqueeze(1)
        target = torch.tril(target_attention)
        attention_mask = (src_attention + target) | torch.tril(raw_attention_mask)
    ...
    def prepare_inputs_for_generation(self, input_ids, **model_kwargs):
        assert "attention_mask" in model_kwargs, "need special attention mask for unilm"
        token_type_ids = model_kwargs["token_type_ids"]
        token_type_ids = torch.cat([token_type_ids, token_type_ids.new_ones((token_type_ids.shape[0], 1))], dim=-1)
        input_shape = input_ids.shape
        effective_batch_size = input_shape[0]
        # TODO 精细化处理type_token_ids 和attention_mask
        assert "mask_token_id" in model_kwargs
        mask_token_id = model_kwargs["mask_token_id"]
        assert "attention_mask" in model_kwargs
        attention_mask = model_kwargs["attention_mask"]
        # 重新构建attention_mask
        attention_mask = torch.cat([attention_mask, attention_mask.new_ones((attention_mask.shape[0], 1))], dim=-1)
        attention_mask = self.create_mask(attention_mask, token_type_ids)
    ...
    def _update_model_kwargs_for_generation(
        ...
        if "token_type_ids" in model_kwargs:
            token_type_ids = model_kwargs["token_type_ids"]
            # TODO Unilm 论文中在省城任务中使用的token type ides =[4,5]
