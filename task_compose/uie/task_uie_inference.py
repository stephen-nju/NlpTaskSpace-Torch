...
import transformers
from datahelper.uie.extraction.record_schema import RecordSchema
from datahelper.uie.sel2record.record import MapConfig
from datahelper.uie.extraction.scorer import *
from datahelper.uie.sel2record.sel2record import SEL2Record
...
class TaskInfer:
    ...
    def __init__(self, model_path, schema_file, max_source_length=256, max_target_length=192) -> None:
        self._tokenizer = transformers.T5TokenizerFast.from_pretrained(model_path)
        self._model = transformers.T5ForConditionalGeneration.from_pretrained(model_path)
        self._model.cuda()
...
def main():
    ...
    parser.add_argument('--model', '-m', default='./models/uie_n10_21_50w_absa_14lap')
