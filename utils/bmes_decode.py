...
def bio_decode(char_label_list: List[Tuple[str, str]]) -> List[Tag]:
    ...
    """
    res = []
    label_list = [a[1] for a in char_label_list]
    char_list = [a[0] for a in char_label_list]
    entity = get_entities(label_list)
    for ent in entity:
        tag = ent[0]
        begin = ent[1]
        end = ent[2]
        term = "".join(char_list[begin:end + 1])
        tag = Tag(term=term,
                  tag=tag,
                  begin=begin,
                  end=end)
