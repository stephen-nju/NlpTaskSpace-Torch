...
def convert_graph(
    ...
    for data_type, instance_list in datasets.items():
        with open(os.path.join(output_folder, f"{data_type}.json"), "w",encoding="utf-8") as output:
...
def convert_to_oneie(output_folder: str, datasets: Dict[str, List[Sentence]]):
    ...
    for data_type, instance_list in datasets.items():
        with open(
            os.path.join(output_folder, f"{data_type}.oneie.json"), "w",encoding="utf-8"
        ) as output:
            for instance in tqdm(instance_list):
                counter.update([f"{data_type} sent"])
                entity_mentions = [
...
def main():
    ...
    parser.add_argument("-config", dest="config", default="data_config/entity_zh/zh_query.yaml")
    parser.add_argument("-output", dest="output", default="entity_zh")
