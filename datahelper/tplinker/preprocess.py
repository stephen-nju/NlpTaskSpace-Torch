...
if __name__ == '__main__':
    ...
    tokenizer = BertTokenizerFast.from_pretrained("E:\\项目资料\\主题挖掘项目\\DataRepository\\bert_model")
    tokenize = tokenizer.tokenize
    get_tok2char_span_map = lambda text: \
        tokenizer.encode_plus(text, return_offsets_mapping=True, add_special_tokens=False)[
            "offset_mapping"]

    pre = Preprocessor(tokenize_func=tokenize, get_tok2char_span_map_func=get_tok2char_span_map)
    data = json.load(open("../../data/tplinker/train.json", encoding="utf-8"))
    data = pre.transform_data(data, ori_format="etl_span", dataset_type="train", add_id=True)
    ...
    if "relation_list" in data[0]:  # train or valid data
        ...
        data = pre.clean_data_wo_span(data, separate=False)
        #         if file_name != "train_data":
        #             set_trace()
        # add char span
        if add_char_span:
            data, miss_sample_list = pre.add_char_span(data, ignore_subword_match=True)
    ...
    data_path = "../../data/tplinker/{}.json".format("train")
    json.dump(data, open(data_path, "w", encoding="utf-8"), ensure_ascii=False)

    rel2id_path = "../../data/tplinker/rel2id.json"
    json.dump(rel2id, open(rel2id_path, "w", encoding="utf-8"), ensure_ascii=False)

    ent2id_path = "../../data/tplinker/ent2id.json"
